...
類似度の比較可能性は「全ベクトルが同一モデル・同一次元で作られていること」だけに依存する。このため埋め込みの生成はインデックス作成側・検索側で単一のモジュールに集約し、モデルと次元をずらせない構造にしている。
...
| エッジ | 検索 API と静的配信を 1 つのデプロイ単位に同居 | Cloudflare Workers |
...
- **検索インデックス（ベクトル DB）**: 複合ベクトルと、検索結果の表示に必要なメタデータ。**商品説明文はここが唯一の置き場所**（生成物であり、インデックスを作り直せば再生成される）
...
検索 API と静的配信をひとつのデプロイ単位にまとめた。デモ規模では分割の運用コストが利得を上回る。静的配信とベクトル DB はどちらも同じエッジ基盤のマネージドサービスで、インフラの管理対象はこの Worker 1 つに閉じる。
...
説明文は AI 生成であり、人が確定させるデータではない。リポジトリにコミットして固定するのではなく、インデックス作成の過程で生成し検索インデックスに保存する。インデックスを作り直せば説明文も作り直される。

## 品質特性

- **秘密情報**: AI API の秘密鍵はエッジの secret のみ。コード・レスポンス・リポジトリに現れない
- **書き込み境界**: 検索インデックスへの書き込みは認証された更新経路に限る。検索は読み取り専用
- **入力制限**: 画像はフォーマットとサイズを検証、テキストは長さを制限。上流 API のレート制限には再試行で適合
- **整合性**: 埋め込みモデル・次元の定義を一元化し、次元不一致はインデックス作成・検索の両方で即座に失敗させる（沈黙した空間のズレを作らない）
- **反映遅延**: ベクトル DB の更新は非同期反映。更新直後の検索は部分的な結果になりうる（デモでは許容）

## 制約と限界

- 商品数はクライアントサイドフィルタが成立する規模（数十〜数百件）を想定。桁が増えたら構造化条件の DB 側適用が最初の変更点
